...
shown in Fig.~\ref{fig-04} driven by the broken rotation symmetry (the MSLs
transform no longer one into another).

\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth,angle=-90]{img/4a.pdf}
	\caption{Asymmetric Fermi surface (orange = circle for reference) when
          green and blue mmoms in Fig.~\ref{fig:totalkagomephase1a}
          are slightly rotated by $\varphi$ (so that red is now the inequivalent MSL).
          Note that by simultaneous rotation of all moments the FS
          does not change, i.e. while the total magnetisation is
          relevant (it can serve as a measure of symmetry breaking), its
          direction is not.
        }
        \label{fig-04}
\end{figure}
